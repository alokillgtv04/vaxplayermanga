...
const CACHE_STORAGE_KEY = 'reader_cache_';
...
// Xác định chương đang xem dựa trên vị trí cuộn
function getCurrentVisibleChapterId() {
  let closestChapterId = null;
...
// Sự kiện scroll cập nhật vị trí đọc và xác định chương hiện tại
viewport.addEventListener('scroll', () => {
  if (current_mode === 'webtoon') {
    const visibleChapterId = getCurrentVisibleChapterId();
    if (visibleChapterId && visibleChapterId !== current_chapter_id) {
      current_chapter_id = visibleChapterId;
      const chap = comicData.chapters.find(c => c.id === visibleChapterId);
      if (chap) {
        comic_title_el.textContent = '📚 ' + chap.title;
        update_read_status(visibleChapterId);
      }
      update_last_read_bookmark();
      // Cập nhật tiêu đề chương hiện tại
    }
    update_progress();
    update_last_read_bookmark();
    // Tự động tải chương kế tiếp nếu cần
    prefetch_next_chapter();
  }
});
